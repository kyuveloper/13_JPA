...
class ProblemOfUsingDirectSQLTests {
    ...
    *
    * 법인 회원과 일반 회원이라는 두 가지 회원을 추상화하여 회원이라는 슈퍼타입 앤터티를 도출하고
    * 서브타입 엔터티로 법인회원과 일반회원이 존재하는 상황인 경우
    * 물리적으로 회원과 법인회원 테이블로 분리된 상황에서는 각각 insert 구문을 수행해야한다
    * insert into 회원..
    * insert into 법인회원
    * 하지만 jpa를 이용하여 상속을 구현하는 경우엔
    * entityManager.persist(법인회원)
    * 이렇게 한 번에 해겨하는 것이 가능하다
    * */

    /*
    * 3-2. 연관관계 문제, 객체 그래프 탐색 문제
    * 객체지향에서 말하는 가지고 있는 (association 연관관계 혹은 Collection 연관관계) 경우 데이터 베이스 저장 구조와 다른 형태이다.
