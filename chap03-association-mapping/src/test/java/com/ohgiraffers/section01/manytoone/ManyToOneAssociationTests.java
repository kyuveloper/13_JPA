...
public class ManyToOneAssociationTests {
    ...
    * 객체 간의 연관 관계를 양방향으로 만들고 싶은 겨우 반대 쪽에서도 필드를 추가해서 참조를 보관하면 된다
    ...
    * */
    ...
    /*
    * ManyToOne 은 다수의 엔티티가 하나의 엔티티를 참조하는 상황에서 상용된다
    * 예를 들어 하나의 카테고리가 여럭 개의 메뉴를 가질 수 있는 상황에서 메뉴 엔티티가 카테고리 엔티티를 참조하는 것이다
    ...
    * join 문법이 sql과는 다소 차이가 있짐나 직접 쿼리를 작성할 수 있는 문법을 제공한다
    * 주의할 점은 FROM 절에 기술할 테이블 명에는 반드시 엔티티 명이 작성되어야 한다
    * */
    @Test
    ...
    * 단, 카태로기라 ㄱ존재하는 값이 아니므로 부모 테이블(tbl_category)에 갑싱 먼저 들어있어야 그 카테고리를 참조하는 자식 테이블에 데이터를 넣을 수 있다
    ...
    * */
    ...
    void 영속성_삭제_테스트(){
        ...
        MenuAndCategory menuAndCategory = entityManager.find(MenuAndCategory.class,100004);
        ...
        transaction.commit();
        MenuAndCategory deleteMenu = entityManager.find(MenuAndCategory.class,100004);
        Category foundCategory = entityManager.find(Category.class, 33337);
